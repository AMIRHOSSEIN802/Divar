...
const { AuthMessage } = require("../../modules/user/user.message");
...
const Authorization = async (req , res , next) => {
    try {
        ...
        if(typeof data === "object" && "id" in data) {
            const user = await UserModel.findById(data.id, {accessToken : 0 , otp : 0 , verifiedMobile:0 , __v:0 , updatedAt: 0}).lean()
